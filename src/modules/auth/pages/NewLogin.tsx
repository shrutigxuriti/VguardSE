...
const NewLogin: React.FC<{ navigation: any }> = ({ navigation }) => {
    ...
    const handleValidation = async () => {
        ...
        //   showLoader(false);
        let validationResponse = { data: { message: "" } }
        validationResponse.data.message = "Please enter OTP to proceed with the login process"
    ...
    const checkOtp = async () => {
        console.log("<><>", number);
        if (otp === '1111') {
            navigation.navigate('setPassword', { usernumber: number })
        }
    }
